...
| 파일 | 역할 |
|------|------|
| `com/jsoncrud/model/Record.java` | id, createdAt, updatedAt, fields(Map) 보유 |
| `com/jsoncrud/repository/JsonFileRepository.java` | JSON 파일 읽기·쓰기 전담 |

### 세부 작업

1. **`Record`** 클래스
   - 필드: `String id`, `String createdAt`, `String updatedAt`, `Map<String, String> fields`
...
1. **Create**
   ...
   - createdAt / updatedAt: `LocalDateTime.now()` ISO-8601 포맷
...
3. **Update**
   - `Record update(String id, String fieldKey, String newValue)`
   - 대상 없으면 `IllegalArgumentException` 발생
   - updatedAt 자동 갱신

4. **Delete**
   - `Record delete(String id)` — 삭제된 레코드 반환 (확인 출력용)
   - 대상 없으면 `IllegalArgumentException` 발생
...
3. **Read 흐름**
   ...
   - 전체 목록: id, createdAt, fields 요약을 테이블로 출력
   - 검색 결과 없으면 안내 메시지 출력

4. **Update 흐름**
   - ID 입력 → 현재 레코드 출력 → 수정할 필드명 입력 → 새 값 입력 → 저장
