...
- 필드명이 레코드에 없으면 값 입력 없이 즉시 `"필드 'xxx' 가 존재하지 않습니다."` 출력 후 메뉴 복귀.
...
| Update 시 존재하지 않는 필드명 | 값 입력 전에 UI에서 선검사 → `"필드 'xxx' 가 존재하지 않습니다."` 출력 후 메뉴 복귀 |
...
- [ ] Update 존재하지 않는 필드명 입력 시 값 입력 없이 즉시 안내 후 메뉴 복귀
- [ ] Delete Y/N 확인 후 삭제 / 취소 분기
- [ ] 숫자 아닌 입력, 없는 ID, 없는 필드명 등 예외 시 앱 종료 없이 메뉴 복귀
